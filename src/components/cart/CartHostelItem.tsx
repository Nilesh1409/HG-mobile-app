...
import type { CartHostelItem as CartHostelItemType } from '../../types/cart.types';
import api from '../../lib/api';
import queryClient from '../../lib/queryClient';

interface Props {
  item: CartHostelItemType;
}

export default function CartHostelItem({ item }: Props) {
  const updateMutation = useMutation({
    mutationFn: (quantity: number) =>
      api.put(`/cart/hostels/${item._id}`, { quantity }),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['cart'] }),
    onError: () => Toast.show({ type: 'error', text1: 'Failed to update' }),
  });

  const deleteMutation = useMutation({
    mutationFn: () => api.delete(`/cart/hostels/${item._id}`),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['cart'] }),
  ...
  const mealLabel = {
    ...
    bedBreakfastAndDinner: 'All Meals',
  }[item.mealOption] ?? item.mealOption;
  ...
  return (
    <View style={styles.card}>
      ...
      <View style={styles.info}>
        <Text style={styles.name} numberOfLines={1}>{item.hostel?.name}</Text>
        <Text style={styles.detail}>{item.roomType} · {mealLabel}</Text>
        <Text style={styles.date}>{item.checkIn} → {item.checkOut}</Text>
      ...
      <TouchableOpacity
        ...
        accessibilityLabel="Remove hostel item"
      >
...
const styles = StyleSheet.create({
  ...
  detail: { fontSize: 12, color: '#f47b20', marginBottom: 2, textTransform: 'capitalize' },
